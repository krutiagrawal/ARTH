...
function GroupLoginForm() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      router.push(searchParams.get('next') || '/group/dashboard')
    } catch {
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <p className="eyebrow text-primary">Group sign in</p>
      <h2 className="font-serif text-4xl md:text-5xl mt-4 leading-tight">Welcome <em className="italic text-primary">back</em>.</h2>
      <p className="mt-3 text-sm text-muted-foreground">For group accounts.</p>
      ...
      <form onSubmit={submit} className="mt-10 space-y-3">
        ...
        <p className="text-xs text-center text-muted-foreground">Not registered yet? <Link href="/group/register" className="text-primary">Register your group</Link></p>
      </form>
    </div>
  )
}

export default function GroupLoginClient() {
  return (
