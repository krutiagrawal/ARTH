...
export default function FollowersClient() {
  ...
  const act = async (followId, action) => {
    try {
      await proxy(`/ngo/followers/${followId}/${action}`, { method: 'POST' })
  ...
  const remove = async () => {
    ...
    try {
      await proxy(`/ngo/followers/${removeTarget.followId}`, { method: 'DELETE' })
  ...
  return (
    <DashboardPageShell className="space-y-6">
      ...
      <ConfirmDialog
        ...
        description="They'll stop following your organization and can re-follow later."
        confirmLabel="Remove"
        destructive
        onConfirm={remove}
      />
