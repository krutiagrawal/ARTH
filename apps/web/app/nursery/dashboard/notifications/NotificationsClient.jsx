...
// Only the nursery-flow types actually appear here in practice — the backend targets each
// notify() call at a specific userId, so a nursery only ever receives the rows below (plus the
// generic post/follow ones any account with a public profile can get). See the mobile app's
// NotificationsScreen for the sibling implementation this copy is kept in sync with.
const TYPE_META = {
  order_placed: { icon: ShoppingCart, text: (n) => `${actorName(n)} placed a new order with you.` },
  reservation_requested: { icon: Handshake, text: (n) => `${actorName(n)} requested saplings from your stock.` },
  stock_low: { icon: AlertTriangle, text: () => 'One of your species is running low on stock.' },
  stock_out_of_stock: { icon: Ban, text: () => 'One of your species just ran out of stock.' },
  bulk_requirement_nearby: { icon: Handshake, text: () => 'An NGO nearby is looking for saplings.' },
  bulk_requirement_response_accepted: { icon: CheckCircle2, text: () => 'Your bulk-supply offer was accepted.' },
  follow_request: { icon: UserPlus, text: (n) => `${actorName(n)} asked to follow you.` },
  new_follower: { icon: UserPlus, text: (n) => `${actorName(n)} started following you.` },
  order_fulfillment_today: { icon: CalendarClock, text: () => 'You have an order due today.' },
  nursery_tree_milestone: { icon: TreePine, text: () => 'A tree you supplied hit a growth milestone.' },
  sapling_planted: { icon: Sprout, text: () => 'A sapling you supplied was planted.' },
  order_plantation_verified: { icon: TreePine, text: () => "This order's saplings are now verified ARTH Trees." },
  nursery_impact_milestone: { icon: TreePine, text: () => 'You hit an impact milestone.' },
