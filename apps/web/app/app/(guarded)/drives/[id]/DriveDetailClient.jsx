...
import { ArrowLeft, CalendarDays, Users, Sprout, Info } from 'lucide-react'
...
export default function DriveDetailClient({ driveId }) {
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/app/drives" className="mb-6 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> Back
      </Link>

      <p className="eyebrow text-primary">{drive.ngoName}</p>
      <h1 className="font-serif text-3xl md:text-4xl mt-2">{drive.title}</h1>
      <p className="mt-3 text-sm text-muted-foreground whitespace-pre-line">{drive.description}</p>

      <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <CalendarDays className="h-4 w-4" /> {new Date(drive.startsAt).toLocaleString()}
          {drive.durationMinutes ? ` · ${drive.durationMinutes} min` : ''}
        </span>
        <span className="flex items-center gap-1.5">
          <Users className="h-4 w-4" /> {drive.confirmedCount} going{drive.capacity != null ? ` of ${drive.capacity} spots` : ''}
        </span>
      </div>
