...
function DonationRow({ d, navigation }: { d: ApiMyDonation; navigation: any }) {
  return (
    <TouchableOpacity onPress={() => navigation.navigate('CampaignDetail', { campaignId: d.campaignId })} activeOpacity={0.85}>
      <BorderCard noPadding style={styles.row}>
        <View style={{ flex: 1 }}>
          <Text style={styles.title}>{d.campaignTitle}</Text>
          <Text style={styles.meta}>{d.ngoName}</Text>
          <Text style={styles.date}>{new Date(d.donatedAt).toLocaleDateString()}</Text>
        </View>
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={styles.amount}>{formatRupees(d.amountCents)}</Text>
          <View style={[styles.statusPill, STATUS_COLOR[d.status]]}>
  ...
  return (
    <View style={styles.container}>
      ...
      {isLoading ? (
        ...
        <EmptyState icon="💚" title="No donations yet" body="Campaigns you donate to will show up here." />
      ) : (
        <ScrollView
