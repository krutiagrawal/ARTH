...
export function AdminNgoApprovalsScreen({ navigation }: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        <Text style={styles.headerTitle}>NGO Approvals</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: bottomClearance }]} showsVerticalScrollIndicator={false}>
        ...
        {ngos.map((ngo, i) => (
          <FadeInRow key={ngo.id} delay={i * 60}>
            <TouchableOpacity
              activeOpacity={0.85}
              onPress={() => navigation.navigate('AdminNgoApprovalDetail', { ngo })}
            >
              <GlassCard variant="dark" style={styles.card}>
                <View style={styles.cardHeaderRow}>
                  <Text style={styles.cardTitle} numberOfLines={1}>{ngo.orgName}</Text>
                  <View style={[styles.statusChip, { borderColor: statusColor(ngo.status) }]}>
