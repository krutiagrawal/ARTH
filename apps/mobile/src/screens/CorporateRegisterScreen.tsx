...
function slugifyHandle(name: string): string {
  return name
    ...
    .replace(/[^a-z0-9_]+/g, '_')
    .replace(/^_+|_+$/g, '')
    .slice(0, 30) || 'corporate';
}

export function CorporateRegisterScreen({ navigation }: any) {
  const { registerCorporate } = useAuth();
  ...
  const handleRegister = useCallback(async () => {
    ...
    try {
      ...
      navigation.reset({ index: 0, routes: [{ name: 'CorporateMain' }] });
    } catch (e) {
      setError(e instanceof ApiError ? e.message : 'Something went wrong. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  }, [companyName, description, industry, city, contactName, email, password, registerCorporate, navigation]);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <BlurCard tint="dark" intensity={40} style={styles.card}>
          ...
          <Text style={styles.subtitle}>
            Your account will be reviewed before your sponsorships go public.
          </Text>
          ...
          <AnimatedButton
            label={isSubmitting ? 'Submitting...' : 'Register Company'}
          ...
          <TouchableOpacity onPress={() => navigation.replace('AccountType')}>
            <Text style={styles.switchText}>
              Not a company? <Text style={styles.switchLink}>Choose a different account type</Text>
            </Text>
          </TouchableOpacity>
        </BlurCard>
      </ScrollView>
