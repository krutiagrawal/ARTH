...
import { browseQuerySchema } from '../schemas/ngosPublic.schema';
import { createReservationSchema } from '../schemas/nursery.schema';
import { BadRequestError } from '../utils/errors';

function serializeNurserySummary(n: any) {
  return { id: n.id, nurseryName: n.nurseryName, description: n.description, logoUrl: n.logoUrl, city: n.city, lat: n.lat, lng: n.lng };
}
...
export default async function nurseriesPublicRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { id: string } }>('/:id', async (request, reply) => {
    reply.send(await nurseryPublicService.getPublicNurseryProfile(fastify.prisma, request.params.id));
  });
